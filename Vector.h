...
inline Vector2D& Vector2D::operator= (const Vector2D& in)
{
	x = in.x;
	y = in.y;
...
inline Vector2D& Vector2D::operator= (float* p)
{
	if (p)
	{
		x = p[0]; y = p[1];
...
inline bool Vector2D::operator! (void) const
{
	return IsZero();
}
...
inline Vector2D Vector2D::operator+ (float* p) const
{
...
public:
	...
	inline float AngleBetween(Vector& other)
	{
		...
		return acos(Dot(other)) * static_cast<float>(180.0 / pi);
	}
