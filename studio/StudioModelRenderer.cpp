...
{
	m_pCvarHiModels = pStudio->GetCvar("cl_himodels");
	m_pCvarMinModels = pStudio->GetCvar("cl_minmodels");
	m_pCvarDeveloper = pStudio->GetCvar("developer");
	m_pCvarDrawEntities = pStudio->GetCvar("r_drawentities");
	m_pCvarClRighthand = pStudio->GetCvar("cl_righthand");
	m_pCvarClShadows = pStudio->GetCvar("cl_shadows");
	m_pCvarClMinCT = pStudio->GetCvar("cl_min_ct");
	m_pCvarClMinT = pStudio->GetCvar("cl_min_t");

	m_pChromeSprite = pStudio->GetChromeSprite();

	pStudio->GetModelCounters(&m_pStudioModelCount, &m_pModelsDrawn);

	m_pbonetransform = (float(*)[MAXSTUDIOBONES][3][4])pStudio->StudioGetBoneTransform();
	m_plighttransform = (float(*)[MAXSTUDIOBONES][3][4])pStudio->StudioGetLightTransform();
	m_paliastransform = (float(*)[3][4])pStudio->StudioGetAliasTransform();
	m_protationmatrix = (float(*)[3][4])pStudio->StudioGetRotationMatrix();
}
...
mstudioanim_t* CStudioModelRenderer::StudioGetAnim(model_t* m_pSubModel, mstudioseqdesc_t* pseqdesc)
{
	...
	if (paSequences == NULL)
	{
		paSequences = (cache_user_t*)pStudio->Mem_Calloc(16, sizeof(cache_user_t));
		m_pSubModel->submodels = (dmodel_t*)paSequences;
	}

	if (!pStudio->Cache_Check((struct cache_user_s*)&(paSequences[pseqdesc->seqgroup])))
	{
		pEngine->Con_DPrintf("loading %s\n", pseqgroup->name);
		pStudio->LoadCacheFile(pseqgroup->name, (struct cache_user_s*)&paSequences[pseqdesc->seqgroup]);
	}
...
void CStudioModelRenderer::StudioFxTransform(cl_entity_t* ent, float transform[3][4])
{
	switch (ent->curstate.renderfx)
	{
		...
		{
			if (pEngine->pfnRandomLong(0, 49) == 0)
			{
				int axis = pEngine->pfnRandomLong(0, 1);

				if (axis == 1)
					axis = 2;

				VectorScale(transform[axis], pEngine->pfnRandomFloat(1, 1.484), transform[axis]);
			} else if (pEngine->pfnRandomLong(0, 49) == 0)
			{
				float offset;
				int axis = pEngine->pfnRandomLong(0, 1);
				...
				offset = pEngine->pfnRandomFloat(-10, 10);
				transform[pEngine->pfnRandomLong(0, 2)][3] += offset;
			}
...
{
	m_pCurrentEntity = pStudio->GetCurrentEntity();

	pStudio->GetTimes(&m_nFrameCount, &m_clTime, &m_clOldTime);
	pStudio->GetViewInfo(m_vRenderOrigin, m_vUp, m_vRight, m_vNormal);
	pStudio->GetAliasScale(&m_fSoftwareXScale, &m_fSoftwareYScale);
	...
	if (m_pCurrentEntity->curstate.renderfx == kRenderFxDeadPlayer)
	{
		...
		if (m_pCurrentEntity->curstate.renderamt <= 0 || m_pCurrentEntity->curstate.renderamt > pEngine->GetMaxClients())
			return 0;

		deadplayer = *(pStudio->GetPlayerState(m_pCurrentEntity->curstate.renderamt - 1));
	...
	m_pStudioHeader = (studiohdr_t*)pStudio->Mod_Extradata(m_pRenderModel);

	pStudio->StudioSetHeader(m_pStudioHeader);
	pStudio->SetRenderModel(m_pRenderModel);

	StudioSetUpTransform(0);

	if (m_pCurrentEntity == pEngine->GetViewModel() && (m_pCvarClRighthand->v))
	{
		(*m_protationmatrix)[0][1] *= -1;
		(*m_protationmatrix)[1][1] *= -1;
		(*m_protationmatrix)[2][1] *= -1;
	
		//pStudio->StudioSetCullState(true);
	}

	if (flags & STUDIO_RENDER)
	{
		if (!pStudio->StudioCheckBBox())
			return 0;
	...
	if (flags & STUDIO_EVENTS)
	{
		...
		pStudio->StudioClientEvents();

		if (m_pCurrentEntity->index > 0)
		{
			cl_entity_t* ent = pEngine->GetEntityByIndex(m_pCurrentEntity->index);
	...
	if (flags & STUDIO_RENDER)
	{
		...
		pStudio->StudioDynamicLight(m_pCurrentEntity, &lighting);

		pStudio->StudioEntityLight(&lighting);
		pStudio->StudioSetupLighting(&lighting);

		m_nTopColor = m_pCurrentEntity->curstate.colormap & 0xFF;
		m_nBottomColor = (m_pCurrentEntity->curstate.colormap & 0xFF00) >> 8;

		pStudio->StudioSetRemapColors(m_nTopColor, m_nBottomColor);
...
int CStudioModelRenderer::StudioDrawPlayer(int flags, entity_state_t* pplayer)
{
	...
	if (m_nPlayerIndex < 0 || m_nPlayerIndex >= pEngine->GetMaxClients())
		return 0;

	m_pRenderModel = pStudio->SetupPlayerModel(m_nPlayerIndex);
	...
	{
		vec3_t orig_angles;
		m_pPlayerInfo = pStudio->PlayerInfo(m_nPlayerIndex);
	...
	if (flags & STUDIO_RENDER)
	{
		if (m_pCvarHiModels->v && m_pRenderModel != m_pCurrentEntity->model)
			m_pCurrentEntity->curstate.body = 255;

		if (!(m_pCvarDeveloper->v == 0 && pEngine->GetMaxClients() == 1) && (m_pRenderModel == m_pCurrentEntity->model))
		...
		if (pplayer->weaponmodel)
		{
			...
			model_t* pweaponmodel = pStudio->GetModelByIndex(pplayer->weaponmodel);

			m_pStudioHeader = (studiohdr_t*)pStudio->Mod_Extradata(pweaponmodel);
			pStudio->StudioSetHeader(m_pStudioHeader);
...
void CStudioModelRenderer::StudioCalcAttachments(void)
{
	...
	if (m_pStudioHeader->numattachments > 4)
		pEngine->Con_DPrintf("Too many attachments on %s\n", m_pCurrentEntity->model->name);
...
void CStudioModelRenderer::StudioRenderModel(float* lightdir)
{
	...
	if (iSaveRenderFx == kRenderFxGlowShell)
	{
		...
		pEngine->pTriAPI->SpriteTexture(m_pChromeSprite, 0);

		pStudio->SetForceFaceFlags(STUDIO_NF_CHROME);
		StudioRenderFinal();
	}
...
void CStudioModelRenderer::StudioRenderFinal_Software(void)
{
	...
	pStudio->SetupRenderer(0);

	if (m_pCvarDrawEntities->v == 2)
	{
		pStudio->StudioDrawBones();
	} else if (m_pCvarDrawEntities->v == 3)
	{
		pStudio->StudioDrawHulls();
	} else
	{
		for (i = 0; i < m_pStudioHeader->numbodyparts; i++)
		{
			pStudio->StudioSetupModel(i, (void**)&m_pBodyPart, (void**)&m_pSubModel);
			pStudio->StudioDrawPoints();
		}
	}

	if (m_pCvarDrawEntities->v == 4)
	{
		pEngine->pTriAPI->RenderMode(kRenderTransAdd);
		pStudio->StudioDrawHulls();
		pEngine->pTriAPI->RenderMode(kRenderNormal);
	}

	if (m_pCvarDrawEntities->v == 5)
		pStudio->StudioDrawAbsBBox();

	pStudio->RestoreRenderer();
}
...
void CStudioModelRenderer::StudioRenderFinal_Hardware(void)
{
	...
	rendermode = pStudio->GetForceFaceFlags() ? kRenderTransAdd : m_pCurrentEntity->curstate.rendermode;
	pStudio->SetupRenderer(rendermode);
	...
	} else
	{
		for (i = 0; i < m_pStudioHeader->numbodyparts; i++)
		{
			...
			pStudio->GL_SetRenderMode(rendermode);
			pStudio->StudioSetRenderamt(m_pCurrentEntity->curstate.renderamt);
			pStudio->StudioDrawPoints();
		}
	}
...
void CStudioModelRenderer::StudioDrawShadow(Vector origin, float scale)
{
	...
	pEngine->pEventAPI->EV_SetUpPlayerPrediction(false, true);
	pEngine->pEventAPI->EV_PushPMStates();
	pEngine->pEventAPI->EV_SetSolidPlayers(-1);
	pEngine->pEventAPI->EV_SetTraceHull(2);
	pEngine->pEventAPI->EV_PlayerTrace(origin, endPoint, PM_STUDIO_IGNORE | PM_GLASS_IGNORE, -1, &pmtrace);
	pEngine->pEventAPI->EV_PopPMStates();
	...
	pStudio->StudioRenderShadow(m_iShadowSprite, p1, p2, p3, p4);
}
