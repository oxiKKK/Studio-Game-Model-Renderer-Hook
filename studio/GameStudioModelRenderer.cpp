...
void CGameStudioModelRenderer::SavePlayerState(entity_state_t* pplayer)
{
	...
	cl_entity_t* ent = pStudio->GetCurrentEntity();
...
float StudioFrameAdvance(client_anim_state_t* st, float framerate, float flInterval)
{
	if (flInterval == 0.0)
	{
		flInterval = (pEngine->GetClientTime() - st->animtime);

		if (flInterval <= 0.001)
		{
			st->animtime = pEngine->GetClientTime();
			return 0.0;
		}
	}
...
void CGameStudioModelRenderer::SetupClientAnimation(entity_state_t* pplayer)
{
	...
	void* pmodel = (studiohdr_t*)pStudio->Mod_Extradata(ent->model);
...
int CGameStudioModelRenderer::StudioDrawPlayer(int flags, entity_state_t* pplayer)
{
	...
	if (m_bLocal && pStudio->GetCurrentEntity() == pEngine->GetLocalPlayer())
		isLocalPlayer = true;
...
bool WeaponHasAttachments(entity_state_t* pplayer)
{
	...
	pweaponmodel = pStudio->GetModelByIndex(pplayer->weaponmodel);
	modelheader = (studiohdr_t*)pStudio->Mod_Extradata(pweaponmodel);
...
int CGameStudioModelRenderer::_StudioDrawPlayer(int flags, entity_state_t* pplayer)
{
	...
	//@todo extra_player_info_t* pExtra
	//extra_player_info_t* pExtra = g_PlayerExtraInfo + pplayer->number;
	//
	//if (m_pCvarMinModels && m_pCvarMinModels->v)
	//{
	//	int team = pExtra->teamnumber;
	//	if (team == TERRORIST)
	//	{
	//		// set leet if model isn't valid
	//		int modelIdx = m_pCvarClMinT && BIsValidTModelIndex(m_pCvarClMinT->v) ? m_pCvarClMinT->v : 1;
	//
	//		m_pRenderModel = pEngine->CL_LoadModel(sPlayerModelFiles[modelIdx], NULL);
	//	} else if (team == CT)
	//	{
	//		if (pExtra->vip)
	//			m_pRenderModel = pEngine->CL_LoadModel(sPlayerModelFiles[3], NULL);
	//		else
	//		{
	//			// set gign, if model isn't valud
	//			int modelIdx = m_pCvarClMinCT && BIsValidCTModelIndex(m_pCvarClMinCT->v) ? m_pCvarClMinCT->v : 2;
	//
	//			m_pRenderModel = pEngine->CL_LoadModel(sPlayerModelFiles[modelIdx], NULL);
	//		}
	//	}
